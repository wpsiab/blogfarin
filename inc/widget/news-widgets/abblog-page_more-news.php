 ...
 * @since 1.0.0
*/
class Abblog_page_more_news extends WP_Widget 
{

    public function __construct() 
    {
        parent::__construct(
            'widget_page_more_news', 
            _x( 'AB Page More News', 'Category Posts Widget' ), 
            [ 'description' => __( 'Display a list of posts from a selected category.' ) ] 
        );
        $this->alt_option_name = 'widget_page_more_news';
    ...
    public function widget( $args, $instance ) 
    {
        ...
        $cat_id         = ( ! empty( $instance['cat_id'] ) ) ? absint( $instance['cat_id'] ) : true;
        $random         = ( ! empty( $instance['rand'] ) ) ? absint( $instance['rand'] ) : false ;
                    ...
                    <div class="media-post-wrap-2 media">
                        ...
                        <div class="media-body">
                            ...
                            <?php  $categories = get_the_category();
                                if ( ! empty( $categories ) ) {
                                    echo '<a class="tag tag-red" href="' . esc_url( get_category_link( $categories[0]->term_id ) ) . '">' . esc_html( $categories[0]->name ) . '</a>';
                                }
                            ?> 
    ...
    public function update( $new_instance, $old_instance ) 
    {
        ...
        $instance['rand']      = isset( $new_instance['rand'] ) ? wp_validate_boolean( $new_instance['rand'] ) : false;
